...
//  Synopsis    [Cycle]
//
//  Description [This class represents a clock cycle, and manages all the
//				 variables that belong to this cycle. All the variables in the 
//				 system are divided to seperated cycles.
//				 All the Cycle instanses are held by the CVariableOrderManager
//				 instanse. All of the cycles, except one, hold state variables. 
//				 One cycle holds non-state variables, which is, gates-logical 
//				 and input variables.
//
//  SeeAlso     []
//
...
class CCycle {
  private:
    int	 _max_score_pos;        // index of the unassigned var with max score
    int  _num_free_variables;   // num of free variables in this cycle
    vector<pair<CVariable*, int> > _ordered_vars;  // pair's first pointing to the var, second is the score.
    int _ordered_vars_itr;      // needed in order to reset the order of the variables

  public:
    inline CCycle() : _max_score_pos(0), _num_free_variables (0), _ordered_vars_itr(0) {}
    void add_variable(CVariable& var);
    void reset_ordered_var(CVariable* var);
    void sort_var_score();
    void update_var_score();
    void decay_variable_score();
    void adjust_variable_order(CVariable& var, int orig_score, int new_score, int bubble_step, CSolverStats* stats);
    void update_available_var_modifications(CVariable& var);
    CVariable* decide_next_var(CSolverParameters* params, CSolverStats*  stats);

    inline void reset_ordered_var_itr() {
      _ordered_vars_itr = 0;
    }

    inline int & num_free_variables() {
      return _num_free_variables;
    }
};
...
// **Class*********************************************************************
//
//  Synopsis    [Variables order manager]
//
//  Description [This class manages the cycles - their order and is the interface
//				 with them from CSolver. In the system there is one instanse 
//				 of this class. Its functions are called by CSolver, and forward
//				 the call to the relevant cycle. 
//				 CVariableOrderManager holds all the cycles, ordered in a DFS 
//				 order. It sets the assignment to the variables, by the order 
//				 of the cycles. All of the cycles, except one, hold state 
//				 variables, which are assigned first. 
//				 One cycle holds non-state variables, which is, gates-logical 
//				 and input variables.
//
//  SeeAlso     []
//
// ****************************************************************************
class CVariableOrderManager {
  private:
    vector<unsigned int> _cycles_decision_order; // Maps the DFS order of cycles 
    vector<unsigned int> _cycles_position;       // An opposite mapping of _cycles_decision_order:
												 // given a cycle index, returns its position in the DFS order.
    vector<CCycle>       _state_cycles;          // All cycles that contain state variables
    CCycle               _non_state_vars_cycle;  // An additional cycle containning all non-state variables 
												 // in the system (inputs + gates).
    unsigned int         _state_cycles_num;      // Number of state variables cycles.
    unsigned int         _cycles_order_pos;      // current cycle to assign its variables

  private:
    void init_cycles_order();
    void init_vars_by_cycles(CSolver* parent);
    unsigned int get_cycles_num(CSolver* parent);
    CCycle& cycle(int idx);

  public:
    static const int NON_STATE_VAR_CYCLE;
    void init(CSolver* parent);
    void reset_ordered_var_itr();
    void reset_ordered_var(CVariable* var);
    void sort_var_score();
    void update_var_score();
    void decay_variable_score();
    void decrease_num_free_variables(CVariable& var);
    void increase_num_free_variables(CVariable& var);
    void update_available_var_modifications(CVariable& var);
    void adjust_variable_order(CVariable& var, int orig_score, int new_score, int bubble_step, CSolverStats* stats);
    CVariable* decide_next_var(CSolverParameters* params, CSolverStats*  stats);
};
...
class CSolver:public CDatabase {
  protected:
    ...
    CVariableOrderManager  _variable_order_manager; //manages order of decisions based on variables' cycle
  ...
  protected:
    ...
    void init_variable_order_manager(void);
    ...
    void update_sort_var_score(void);
  ...
  public:
    ...
    void set_cycle_idx(vector<pair<int, int> >& variables_cycles);
    ...
    void print_vars(ostream & os = cout);
