...
//    cout <<"read cnf "<<endl;
   ...
   vector<pair<int, int> > variables_cycles;
    while (inp.getline(line_buffer, MAX_LINE_LENGTH)) {
        ++ line_num;
		// parse the comments, in order to know to which cycle a variable
		// belongs to. Then, insert all variables to the database.
        if (line_buffer[0] == 'c') { 
			// an example to a state variable (with id 20. In cycle 5)
			// c 20 = V5_G18[0]
			// an example to a not state variable
			// c 15 = S4[0]
			char type_char;
			...
			int arg = sscanf (line_buffer, "c %d = %c%d%s", &var_num, &type_char, &cycle_num, tmp_str);
			if (arg < 4) {
				cerr << "Unable to read a variable comment at line " << line_num << endl;
				exit(3);
			}
			if (strstr(tmp_str, "[0]") == NULL) {
				cerr << "Unable to read a variable comment at line " << line_num << endl;
				exit(3);
			}

			if (type_char == 'V') {
				pair<int, int> cycletmp(var_num, cycle_num);
				variables_cycles.push_back(cycletmp);
			}
			continue; 
        }
   ...
   SAT_SetCycleIdx(mng, variables_cycles);
